...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function == LossFunction.LogCosh:
            return np.log(np.cosh(x @ self.w - y))
        elif self.loss_function == LossFunction.MAE:
            return 1/x.shape[0] * np.abs(x @ self.w - y).sum()
            ...
            residual = self.predict(x) - y
            return residual.T @ residual / x.shape[0]
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        sample = np.random.choice(range(x.shape[0]), self.batch_size, replace=False)
        batch_x = x[sample]
        batch_y = y[sample]
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        w_diff = self.lr() * m_hat/(np.sqrt(v_hat) + self.eps)
        self.w -= w_diff
        return -w_diff
...
class AdaMaxReg(BaseDescentReg, AdaMax):
    """
    Adaptive gradient algorithm with inf norm with regularization class
    """
